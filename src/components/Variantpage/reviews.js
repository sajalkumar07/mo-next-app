...
import Review from "../Productpage/Structure/share-reviews";
import SlidingPagination from "../Homepage/slidingPagination.js";
...
const Reviews = (props, ref) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIdos(data?.data?.product_id?._id || null);
      } catch (error) {
    ...
    const fetchReviews = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}/api/v1/tern/get-review/${idos}`
        );
  ...
  // Handle scroll for dots navigation
  useEffect(() => {
    const handleScroll = () => {
