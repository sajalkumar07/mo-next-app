    ...
    const fetchCarData = async () => {
      try {
        const response = await fetch(
          `${apiBaseUrl}/api/varient/getbyid/${params.id}`
        );
        const data = await response.json();
        setCarName(data?.data?.product_id?.carname);
        fetchVideos(data?.data?.product_id?.carname);
      } catch (error) {
      ...
      if (carName?.trim()) {
        try {
          ...
          if (data.data && Array.isArray(data.data) && data.data.length > 0) {
            ...
            // console.warn('No videos found for the car name in the specified channel.');
            setLatestVideos([]);
          }
        } catch (error) {
    ...
    <div className="mt-4">
      ...
      >
        ...
        <div className="w-full max-w-screen-xl mx-auto px-2">
          <div className="w-full overflow-x-auto scroll-smooth pb-4 scrollbar-thin scrollbar-track-gray-100 scrollbar-hide">
