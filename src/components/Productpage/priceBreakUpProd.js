...
const ViewPriceBreakup = ({ product, brand, productname }) => {
  ...
  const fetchProductsForBrand = async (brandId) => {
    try {
      ...
      const data = await response.json();
      console.log("Products fetched for brand:", data); // Debug log
    ...
    <div>
      ...
      {showPopup && (
        <div style={popupStyles.overlay} className="text-dark">
          <div style={popupStyles.popup} className="the-upperpoptuo">
            <button
              className="thecanclebtton"
